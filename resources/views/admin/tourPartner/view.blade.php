            ...
            <div class="pageAdminWithRightSidebar_main">
                <div class="pageAdminWithRightSidebar_main_content">
                    ...
                    <div class="pageAdminWithRightSidebar_main_content_item">
                        <div class="card">
                            <!-- Thông tin đối tác -->
                            @include('admin.tourPartner.formInfo')
                        </div>
                    </div>
                    <!-- END:: Main content -->
                </div>
                <div class="pageAdminWithRightSidebar_main_rightSidebar">
                    ...
                    <div class="pageAdminWithRightSidebar_main_rightSidebar_item buttonAction" style="padding-bottom:1rem;">
                        <a href="{{ route('admin.tourPartner.list') }}" type="button" class="btn btn-secondary waves-effect waves-float waves-light">Quay lại</a>
                    ...
                    <div class="customScrollBar-y" style="height: calc(100% - 70px);border-top: 1px dashed #adb5bd;">
                        ...
                        <div class="pageAdminWithRightSidebar_main_rightSidebar_item">
                            @include('admin.tourPartner.formLogo')
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts-custom')
    <script type="text/javascript">
        ...
        function addAndUpdateContactForPartner(){
            ...
            if(tmp==''){
                ...
                if(dataForm['partner_contact_id']==null){
                    /* insert */
                    $.ajax({
                        url         : '{{ route("admin.tourPartner.createContact") }}',
                        type        : 'post',
                        dataType    : 'html',
                        data        : {
                            '_token'    : '{{ csrf_token() }}',
                    ...
                    /* update */
                    $.ajax({
                        url         : '{{ route("admin.tourPartner.updateContact") }}',
                        type        : 'post',
                        dataType    : 'html',
                        data        : {
                            '_token'    : '{{ csrf_token() }}',
        ...
        function loadContactOfPartner(idWrite){
            $.ajax({
                url         : '{{ route("admin.tourPartner.loadContact") }}',
                type        : 'post',
                dataType    : 'html',
                data        : {
                    '_token'    : '{{ csrf_token() }}',
        ...
        function loadFormPartnerContact(id){
            $.ajax({
                url         : '{{ route("admin.tourPartner.loadFormContact") }}',
                type        : 'post',
                dataType    : 'json',
                data        : {
                    '_token'    : '{{ csrf_token() }}',
        ...
        function deletePartnerContact(id){
            if(confirm('{{ config("admin.alert.confirmRemove") }}')) {
                $.ajax({
                    url         : '{{ route("admin.tourPartner.deleteContact") }}',
                    type        : 'post',
                    dataType    : 'html',
                    data        : {
                        '_token'    : '{{ csrf_token() }}',
