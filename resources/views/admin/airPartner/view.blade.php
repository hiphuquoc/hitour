...
@section('content')
    @php
        $titlePage      = 'Thêm Hãng máy bay mới';
        $submit         = 'admin.airPartner.create';
        $checkImage     = 'required';
        if(!empty($type)&&$type=='edit'){
            $titlePage  = 'Chỉnh sửa Hãng máy bay';
            $submit     = 'admin.airPartner.update';
            $checkImage = null;
        }
    @endphp
            ...
            <div class="pageAdminWithRightSidebar_main">
                <div class="pageAdminWithRightSidebar_main_content">
                    ...
                    <div class="pageAdminWithRightSidebar_main_content_item">
                        <div class="card">
                            <!-- Thông tin đối tác -->
                            @include('admin.airPartner.formInfo')
                        </div>
                        <div class="card">
                            <!-- Danh sách liên hệ -->
                ...
                <div class="pageAdminWithRightSidebar_main_rightSidebar">
                    ...
                    <div class="pageAdminWithRightSidebar_main_rightSidebar_item buttonAction" style="padding-bottom:1rem;">
                        <a href="{{ route('admin.airPartner.list') }}" type="button" class="btn btn-secondary waves-effect waves-float waves-light">Quay lại</a>
...
@push('scripts-custom')
    <script type="text/javascript">
        ...
        function addAndUpdateContactForPartner(){
            ...
            if(tmp==''){
                ...
                if(dataForm['partner_contact_id']==null){
                    /* insert */
                    $.ajax({
                        url         : '{{ route("admin.airPartner.createContact") }}',
                        type        : 'post',
                        dataType    : 'html',
                        data        : {
                            '_token'    : '{{ csrf_token() }}',
                    ...
                    /* update */
                    $.ajax({
                        url         : '{{ route("admin.airPartner.updateContact") }}',
                        type        : 'post',
                        dataType    : 'html',
                        data        : {
                            '_token'    : '{{ csrf_token() }}',
        ...
        function loadContactOfPartner(idWrite){
            $.ajax({
                url         : '{{ route("admin.airPartner.loadContact") }}',
                type        : 'post',
                dataType    : 'html',
                data        : {
                    '_token'    : '{{ csrf_token() }}',
        ...
        function loadFormPartnerContact(id){
            $.ajax({
                url         : '{{ route("admin.airPartner.loadFormContact") }}',
                type        : 'post',
                dataType    : 'json',
                data        : {
                    '_token'    : '{{ csrf_token() }}',
        ...
        function deletePartnerContact(id){
            if(confirm('{{ config("admin.alert.confirmRemove") }}')) {
                $.ajax({
                    url         : '{{ route("admin.airPartner.deleteContact") }}',
                    type        : 'post',
                    dataType    : 'html',
                    data        : {
                        '_token'    : '{{ csrf_token() }}',
