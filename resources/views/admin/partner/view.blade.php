...
@section('content')
    @php
        ...
        $submit         = 'admin.partner.create';
        $checkImage     = 'required';
        if(!empty($type)&&$type=='edit'){
            $titlePage  = 'Chỉnh sửa Đối tác';
            $submit     = 'admin.partner.update';
            $checkImage = null;
        }
    @endphp
            ...
            <div class="pageAdminWithRightSidebar_main">
                <div class="pageAdminWithRightSidebar_main_content">
                    ...
                    <div class="pageAdminWithRightSidebar_main_content_item">
                        <div class="card">
                            <!-- Thông tin đối tác -->
                            @include('admin.partner.formInfo')
                        </div>
                    </div>
                    <!-- END:: Main content -->
                </div>
                <div class="pageAdminWithRightSidebar_main_rightSidebar">
                    ...
                    <div class="customScrollBar-y" style="height: calc(100% - 70px);border-top: 1px dashed #adb5bd;">
                        ...
                        <div class="pageAdminWithRightSidebar_main_rightSidebar_item">
                            @include('admin.partner.formLogo')
                        </div>
                    </div>
                </div>
            </div>
    ...
    @csrf
        <!-- Input Hidden -->
        <input type="hidden" id="partner_id" name="partner_id" value="{{ $item->id ?? null }}" />
...
@push('scripts-custom')
    <script type="text/javascript">
        ...
        function addAndUpdateContactForPartner(){
            ...
            if(tmp==''){
                ...
                if(dataForm['partner_contact_id']==null){
                    /* insert */
                    $.ajax({
                        url         : '{{ route("admin.partner.createContact") }}',
                        type        : 'post',
                        dataType    : 'html',
                        data        : {
                            '_token'    : '{{ csrf_token() }}',
                    ...
                    /* update */
                    $.ajax({
                        url         : '{{ route("admin.partner.updateContact") }}',
                        type        : 'post',
                        dataType    : 'html',
                        data        : {
                            '_token'    : '{{ csrf_token() }}',
        ...
        function loadContactOfPartner(idWrite){
            $.ajax({
                url         : '{{ route("admin.partner.loadContact") }}',
                type        : 'post',
                dataType    : 'html',
                data        : {
                    '_token'    : '{{ csrf_token() }}',
        ...
        function loadFormPartnerContact(id){
            $.ajax({
                url         : '{{ route("admin.partner.loadFormContact") }}',
                type        : 'post',
                dataType    : 'json',
                data        : {
                    '_token'    : '{{ csrf_token() }}',
        ...
        function deletePartnerContact(id){
            $.ajax({
                url         : '{{ route("admin.partner.deleteContact") }}',
                type        : 'post',
                dataType    : 'html',
                data        : {
                    '_token'    : '{{ csrf_token() }}',
