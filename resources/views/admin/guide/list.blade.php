...
<div class="titlePage">Danh sách Cẩm nang du lịch</div>
<!-- ===== START: SEARCH FORM ===== -->
<form id="formSearch" method="get" action="{{ route('admin.guide.list') }}">
    <div class="searchBox">
            ...
            <tbody>
                ...
                if(!empty($list)){
                    ...
                    foreach($list as $item){
                        echo view('admin.guide.oneRow', ['item' => $item, 'no' => $i]);
                        ++$i;
                    }
                }
                @endphp
...
<!-- Nút thêm -->
<a href="{{ route('admin.guide.view') }}" class="addItemBox">
    ...
    <script type="text/javascript">
        function deleteItem(id){
            if(confirm('{{ config("admin.alert.confirmRemove") }}')) {
                $.ajax({
                    url         : "{{ route('admin.guide.delete') }}",
                    type        : "GET",
                    dataType    : "html",
                    data        : { id : id }
                }).done(function(data){
                    if(data==true) $('#guide_'+id).remove();
                });
            }
        }
