...
<!-- ===== Card Header ===== -->
<div class="titlePage">
    Danh sách Đối tác Hotel
</div>
<!-- ===== START: SEARCH FORM ===== -->
<form id="formSearch" method="get" action="{{ route('admin.hotelPartner.list') }}">
    <div class="searchBox">
        ...
        <div class="searchBox_item" style="margin-left:auto;text-align:right;">
            <?php
                $xhtmlSettingView   = \App\Helpers\Setting::settingView('viewComboPartner', [20, 50, 100, 200, 500], $viewPerPage, $list->total());
                    ...
                    <tr id="item-{{ $item->id }}">
                        ...
                        <td style="vertical-align:top;display:flex;font-size:0.95rem;">
                            <div class="icon-wrapper iconAction">
                                <a href="{{ route('admin.hotelPartner.view', ['id' => $item->id]) }}">
                                    <i data-feather='edit'></i>
                                    <div>Sửa</div>
                                </a>
                            </div>
                            <div class="icon-wrapper iconAction">
                                <a href="{{ route('admin.hotelPartner.view', ['id' => $item->id, 'type' => 'copy']) }}">
...
<!-- Nút thêm -->
<a href="{{ route('admin.hotelPartner.view') }}" class="addItemBox">
    ...
    <script type="text/javascript">
        function deleteItem(id){
            if(confirm('{{ config("admin.alert.confirmRemove") }}')) {
                $.ajax({
                    url         : "{{ route('admin.hotelPartner.delete') }}",
                    type        : "GET",
                    dataType    : "html",
                    data        : { id : id }
                }).done(function(data){
