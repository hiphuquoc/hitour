<input type="hidden" name="tour_country_id" value="{{ !empty($item->id)&&$type=='edit' ? $item->id : null }}" />
...
<div class="formBox">
    <div class="formBox_full">
        ...
        <!-- One Row -->
        <div class="formBox_full_item">
            <label class="form-label inputRequired" for="tour_continent_id">Châu lục</label>
            <select class="select2 form-select select2-hidden-accessible" id="tour_continent_id" name="tour_continent_id">
                <option value="0">- Lựa chọn -</option>
                @if(!empty($tourContinents)&&$tourContinents->isNotEmpty())
                    @foreach($tourContinents as $tourContinent)
                        @php
                            $selected   = null;
                            if(!empty($item->tour_continent_id)&&$item->tour_continent_id==$tourContinent->id) $selected = ' selected';
                        @endphp
