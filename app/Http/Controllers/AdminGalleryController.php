...
class AdminGalleryController extends Controller {
    ...
    public static function uploadGallery($arrayImage, $params = null){
        $result     = [];
        if(!empty($arrayImage)){
            // ===== folder upload
            ...
            foreach($arrayImage as $image){
                ...
                $filename       = $folderUpload.$name.'-gallery-'.time().'-'.$i.'.'.$extension;
    ...
    public static function removeGallery(Request $request){
    ...
    public static function removeGalleryById($id){
        if(!empty($id)){
            try {
                DB::beginTransaction();
                /* xóa file */
