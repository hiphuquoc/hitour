...
class TourCountryRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        return [
            ...
            'tour_continent_id'         => 'integer|required',
                ...
                function($attribute, $value, $fail){
                    ...
                    if(!empty($slug)){
                        $dataCheck  = DB::table('seo')
                                        ->join('tour_country', 'tour_country.seo_id', '=', 'seo.id')
                                        ->select('seo.slug', 'tour_country.id')
                                        ->where('slug', $slug)
                                        ->first();
                        if(!empty($dataCheck)){
                            if(empty(request('tour_country_id'))) $fail('Dường dẫn tĩnh đã trùng với một Điểm đến tour khác trên hệ thống!');
                        }
                    }
                }
            ],
    ...
    public function messages()
    {
        return [
            ...
            'tour_continent_id'         => 'Châu lục không được để trống!',
