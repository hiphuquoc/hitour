...
class TourContinentRequest extends FormRequest
{
    /**
                ...
                function($attribute, $value, $fail){
                    ...
                    if(!empty($slug)){
                        $dataCheck  = DB::table('seo')
                                        ->join('tour_continent', 'tour_continent.seo_id', '=', 'seo.id')
                                        ->select('seo.slug', 'tour_continent.id')
                                        ->where('slug', $slug)
                                        ->first();
                        if(!empty($dataCheck)){
                            if(empty(request('tour_continent_id'))) $fail('Dường dẫn tĩnh đã trùng với một Điểm đến tour khác trên hệ thống!');
                        }
                    }
                }
            ],
