                ...
                function($attribute, $value, $fail){
                    ...
                    if(!empty($slug)){
                        $dataCheck  = DB::table('seo')
                                        ->join('service_location', 'service_location.seo_id', '=', 'seo.id')
                                        ->select('seo.slug', 'service_location.id')
                                        ->where('slug', $slug)
                                        ->first();
                        if(!empty($dataCheck)){
                            if(empty(request('service_location_id'))) $fail('Dường dẫn tĩnh đã trùng với một Điểm đến dịch vụ khác trên hệ thống!');
                        }
                    }
                }
            ],
