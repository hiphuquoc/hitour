                ...
                function($attribute, $value, $fail){
                    ...
                    if(!empty($slug)){
                        $dataCheck  = DB::table('seo')
                                        ->join('blog_info', 'blog_info.seo_id', '=', 'seo.id')
                                        ->select('seo.slug', 'blog_info.id')
                                        ->where('slug', $slug)
                                        ->first();
                        if(!empty($dataCheck)){
                            if(empty(request('blog_info_id'))) $fail('Dường dẫn tĩnh đã trùng với một Chuyên mục khác trên hệ thống!');
                        }
                    }
                }
            ],
